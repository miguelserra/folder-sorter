...
Instalação:
    pip install google-generativeai python-docx python-pptx pypdf
"""
...
from config import (
    PASTA_ORIGEM, CATALOGO_PATH, API_KEY_GEMINI, MODELO_GEMINI,
...
# ---------------------------------------------------------------------------
# CLASSIFICAÇÃO EM LOTE COM GEMINI
# ---------------------------------------------------------------------------
...
def classificar_lote(documentos: list) -> list:
    ...
    texto_resp = re.sub(r"^```json|```$", "", resposta.text.strip(), flags=re.MULTILINE)
    try:
        return json.loads(texto_resp)
    except json.JSONDecodeError:
        print("⚠️  Falha a interpretar resposta do Gemini neste lote — a saltar.")
